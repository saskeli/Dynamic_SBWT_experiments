...
def build_stats(fasta_file, k, prefix_bits=24):
    ...
    data["SSHash"]["size"] = get_filesize(run_sshash.index_path(fasta_file))
    time, mem = run_sbwt.build(fasta_file, k)
    data["SBWT"] = {"time": time, "mem": mem}
    data["SBWT"]["size"] = get_filesize(run_sbwt.index_path(fasta_file))
    time, mem = run_bifrost.build(fasta_file, k)
    data["Bifrost"] = {"time": time, "mem": mem}
    data["Bifrost"]["size"] = get_filesize(run_bifrost.index_path(fasta_file))
    time, mem = run_bufboss.build(fasta_file, k)
    data["BufBOSS"] = {"time": time, "mem": mem}
    # data["BufBOSS"]["size"] = get_filesize(run_bufboss.index_path(fasta_file))
    time, mem = run_dynboss.build(fasta_file, k)
    data["DynamicBOSS"] = {"time": time, "mem": mem}
    data["DynamicBOSS"]["size"] = get_filesize(run_dynboss.index_path(fasta_file))
    time, mem = run_hashset.build(fasta_file, k)
    data["HashSet"] = {"time": time, "mem": mem}
    data["HashSet"]["size"] = get_filesize(run_hashset.index_path(fasta_file))
    data["k"] = k
...
def merge_stats(indexed_file, other_indexed_file):
    ...
    data["CBL"] = {"time": time, "mem": mem}
    time, mem = run_hashset.merge(indexed_file, other_indexed_file)
    data["HashSet"] = {"time": time, "mem": mem}
    data["kmers"] = run_cbl.count(indexed_file)
    data["bytes"] = get_filesize(indexed_file)
    data["query_kmers"] = run_cbl.count_query(indexed_file, other_indexed_file)
...
def intersect_stats(indexed_file, other_indexed_file):
    ...
    data["CBL"] = {"time": time, "mem": mem}
    time, mem = run_hashset.intersect(indexed_file, other_indexed_file)
    data["HashSet"] = {"time": time, "mem": mem}
    data["kmers"] = run_cbl.count(indexed_file)
    data["bytes"] = get_filesize(indexed_file)
    data["query_kmers"] = run_cbl.count_query(indexed_file, other_indexed_file)
